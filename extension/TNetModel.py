...
MFParams = namedtuple('MFParams', 'batch_size, rev_max_len, embedding_size, user_embedding_size, FM_k, '
                            ...
                            'train_epochs, val_epochs, test_epochs, num_filters,review_delim,review_emb')
...
class TNetModel(object):
    ...
    """
    def __init__(self, mfp, sequence_length, rev_sequence_length, word_vocab_size, embedding_size, filter_sizes, num_filters, trans_layers):
        self.mfp = mfp
        
        self.word_vocab_size = word_vocab_size
        self.max_seq_len = rev_sequence_length
        self.trans_layers = trans_layers
        # Placeholders for input, output and dropout
        
        self.input_usertext = tf.placeholder(tf.float32, [None, sequence_length,embedding_size], name="input_usertext") #change #
        self.input_itemtext = tf.placeholder(tf.float32, [None, sequence_length,embedding_size], name="input_itemtext") #Change#
        self.input_rating = tf.placeholder(tf.float32, [None], name="input_rating")
        self.input_review = tf.placeholder(tf.int32, [None, rev_sequence_length], name="input_revABtext")
        ...
        self.embedded_utextx = tf.expand_dims(self.input_usertext, -1)
        self.embedded_btextx = tf.expand_dims(self.input_itemtext, -1)
        ...
        with tf.variable_scope(Tscope):
            ...
            for _, filter_size in enumerate(filter_sizes):
                with tf.name_scope("conv-maxpool-user-%s" % filter_size):
                    ...
                    conv = tf.nn.conv2d(
                        self.embedded_utextx,#self.input_usertext,
                        W_user,
                        strides=[1, 1, 1, 1],
                        padding="VALID",
                        name="conv")
...
#             user_drop_rep = tf.nn.dropout(user_rep, self.dropout_keep_prob)
            ...
            for _, filter_size in enumerate(filter_sizes):
                with tf.name_scope("conv-maxpool-item-%s" % filter_size):
                    ...
                    conv = tf.nn.conv2d(
                        self.embedded_btextx,#self.input_itemtext,#
                        W_item,
                        strides=[1, 1, 1, 1],
                        padding="VALID",
                        name="conv")
        ...
        with tf.variable_scope(Ascope):
            ...
            for _, filter_size in enumerate(filter_sizes):
                with tf.name_scope("conv-maxpool-rev-%s" % filter_size):
                    ...
                    pooled = tf.nn.max_pool(
                        h_rev,
                        ksize=[1, rev_sequence_length - filter_size + 1, 1, 1],
                        strides=[1, 1, 1, 1],
                        padding='VALID',
                        name="pool")
...
#             oth_loss = tf.reduce_sum(tf.minimum(tf.abs(self.oth_prediction - self.input_rating), tf.abs(self.oth_prediction - self.act_prediction))) #L1 loss
    ...
    def run_train_step(self, sess, input_rating, user_intlist, item_intlist, rev_intlist, dp):
        ...
        input_usertext, input_itemtext = user_intlist, item_intlist#self._proc_input(user_intlist, item_intlist)
