...
class DataPairMgr():
    ...
    def get_int_review(self, user, item):    
        if (user,item) in self.user_item_map:
            return self._int_list(self.user_item_map[(user,item)])
        else:
            return [0]
    ...
    def get_int_review_rating(self, user, item):    
        if (user,item) in self.user_item_map:
            return self._int_list(self.user_item_map[(user,item)]), self.user_item_rating[(user,item)]
        else:
